...
import {
  ...
  loadAccounts, addAccount, updateAccount, deleteAccount, bulkDeleteAccounts, reorderAccounts, updateAccountAssignees,
...
import { EditGameModal, EditIdPassModal, EditAccountModal, EditNoticeModal, EditSubAdminModal } from './EditModals';
...
// v20: form for adding a new non-game account (social, VPN, financial, etc.)
function AccountsEntryForm({ subs, onSubmit }: any) {
  const init = () => ({
    platform: '', accountName: '', username: '', password: '',
    recoveryEmail: '', phone: '', backupCodes: '', notes: '',
    assignees: [] as string[],
  });
  const [v, setV] = useState<any>(init);
  const [busy, setBusy] = useState(false);
  ...
  const handle = async () => {
    setErr('');
    if (!v.platform.trim() || !v.username.trim() || !v.password) {
      return setErr('Platform, username, and password are required');
    }
    if (v.assignees.length === 0) return setErr('Assign to at least one sub-admin or "Assign to all"');
    setBusy(true);
    try { await onSubmit(v); setV(init()); setShowPass(false); setShowOptional(false); }
    catch (e: any) { setErr(friendlyError(e)); }
    finally { setBusy(false); }
  };
  return (
    <div style={{ ...S.softCard, marginBottom: '1.25rem' }}>
      <div className="infos-grid2" style={S.grid2}>
        <div><label style={S.label}>Platform</label><TextInput value={v.platform} onChange={(e: any) => setV({ ...v, platform: e.target.value })} placeholder="Facebook, NordVPN…" /></div>
        <div><label style={S.label}>Account name (optional)</label><TextInput value={v.accountName} onChange={(e: any) => setV({ ...v, accountName: e.target.value })} placeholder="Personal, Work…" /></div>
      </div>
      <div className="infos-grid2" style={{ ...S.grid2, marginTop: '10px' }}>
        <div><label style={S.label}>Username / Email</label><TextInput value={v.username} onChange={(e: any) => setV({ ...v, username: e.target.value })} placeholder="Login identifier" /></div>
        <div>
          <label style={S.label}>Password</label>
          <div style={{ position: 'relative' }}>
            <TextInput type={showPass ? 'text' : 'password'} value={v.password} onChange={(e: any) => setV({ ...v, password: e.target.value })} placeholder="Main password" style={{ paddingRight: '60px' }} />
            <button type="button" onClick={() => setShowPass(!showPass)}
              style={{ position: 'absolute', right: '6px', top: '50%', transform: 'translateY(-50%)', background: 'transparent', border: 'none', fontSize: '11.5px', color: C.textSecondary, cursor: 'pointer', padding: '4px 8px', fontWeight: 500 }}>
              {showPass ? 'Hide' : 'Show'}
            </button>
      ...
      <div style={{ marginTop: '10px' }}>
        <button type="button" onClick={() => setShowOptional(!showOptional)}
          style={{ background: 'transparent', border: 'none', color: C.accentText, fontSize: '12.5px', fontWeight: 600, cursor: 'pointer', padding: '6px 0' }}>
          {showOptional ? '− Hide optional fields' : '+ Add recovery email, phone, backup codes, notes'}
        </button>
      </div>
      {showOptional && (
        <>
          <div className="infos-grid2" style={{ ...S.grid2, marginTop: '8px' }}>
            <div><label style={S.label}>Recovery email</label><TextInput type="email" value={v.recoveryEmail} onChange={(e: any) => setV({ ...v, recoveryEmail: e.target.value })} placeholder="backup@example.com" /></div>
            <div><label style={S.label}>Phone</label><TextInput value={v.phone} onChange={(e: any) => setV({ ...v, phone: e.target.value })} placeholder="+977…" /></div>
          </div>
          <div style={{ marginTop: '10px' }}>
            <label style={S.label}>2FA backup codes</label>
            <TextArea value={v.backupCodes} onChange={(e: any) => setV({ ...v, backupCodes: e.target.value })} placeholder="Paste all backup codes here, one per line" />
          </div>
          <div style={{ marginTop: '10px' }}>
            <label style={S.label}>Notes</label>
            <TextArea value={v.notes} onChange={(e: any) => setV({ ...v, notes: e.target.value })} placeholder="Anything extra to remember" />
          </div>
        </>
      )}
      <div style={{ marginTop: '12px' }}>
...
// ---------------- v20: Tab — Accounts (non-game credentials) ----------------
function AccountsTabInner({ user, subs, entries, setEntries, reload }: any) {
  const isAdmin = isAdminRole(user.role);
  const [confirmEl, confirm] = useConfirm();
  const [reveal, setReveal] = useState<any>({});
  const [revealCodes, setRevealCodes] = useState<any>({});
  const [q, setQ] = useState('');
  const [filterSub, setFilterSub] = useState<'all' | string>('all');
  const [editing, setEditing] = useState<any>(null);
  const [selectMode, setSelectMode] = useState(false);
  ...
  const visible = useMemo(() => {
    ...
    return base.filter((e: any) =>
      ...
      (e.username || '').toLowerCase().includes(s) ||
      (e.recoveryEmail || '').toLowerCase().includes(s) ||
  ...
  const add = async (vals: any) => {
    const newEntry = {
      ...vals,
      platform: (vals.platform || '').trim(),
      accountName: (vals.accountName || '').trim(),
      username: (vals.username || '').trim(),
      // password intentionally NOT trimmed
      recoveryEmail: (vals.recoveryEmail || '').trim(),
      phone: (vals.phone || '').trim(),
      backupCodes: (vals.backupCodes || '').trim(),
    ...
    setEntries((prev: any[]) => [...prev, newEntry]);
    try { await addAccount(newEntry); }
    catch (e: any) { setEntries((prev: any[]) => prev.filter(x => x.id !== newEntry.id)); throw e; }
  };
  const del = async (e: any) => {
    const label = e.platform + (e.accountName ? ` (${e.accountName})` : '');
    const ok = await confirm({ title: `Delete account "${label}"?`, message: 'This cannot be undone.', confirmLabel: 'Delete', danger: true });
    if (!ok) return;
    const deletedItem = e;
    setEntries((prev: any[]) => prev.filter(x => x.id !== e.id));
    try { await deleteAccount(e.id); }
    catch (err: any) {
      setEntries((prev: any[]) => [...prev, deletedItem].sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)));
      alert(friendlyError(err));
    }
  ...
  const saveEdit = async (patch: any) => {
    ...
    const originalItem = entries.find((x: any) => x.id === id);
    setEntries((prev: any[]) => prev.map((x: any) => x.id === id ? { ...x, ...patch, updatedAt: Date.now() } : x));
    try { await updateAccount(id, patch); }
    catch (e: any) {
      if (originalItem) setEntries((prev: any[]) => prev.map((x: any) => x.id === id ? originalItem : x));
      throw e;
    }
  ...
  const reorder = async (no: any[]) => {
    ...
    const keptIds = new Set(reordered.map((e: any) => e.id));
    setEntries((prev: any[]) => [...reordered, ...prev.filter((e: any) => !keptIds.has(e.id))]);
    try { await reorderAccounts(no.map((e: any) => e.id)); }
    catch (err: any) {
      setEntries((prev: any[]) => prev.map((x: any) => {
        const o = originalOrder.find(r => r.id === x.id);
        return o ? { ...x, sortOrder: o.sortOrder } : x;
  ...
  const bulkDelete = async () => {
    const ok = await confirm({ title: `Delete ${selected.length} accounts?`, message: 'This cannot be undone.', confirmLabel: `Delete ${selected.length}`, danger: true });
    if (!ok) return;
    const toDelete = [...selected];
    const deletedItems = entries.filter((x: any) => toDelete.includes(x.id));
    setEntries((prev: any[]) => prev.filter(x => !toDelete.includes(x.id)));
    setSelected([]); setSelectMode(false);
    try { await bulkDeleteAccounts(toDelete); }
    catch (err: any) {
      setEntries((prev: any[]) => [...prev, ...deletedItems].sort((a: any, b: any) => (a.sortOrder || 0) - (b.sortOrder || 0)));
      alert(friendlyError(err));
    }
  ...
  const renderItem = (e: any) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '10px' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600, fontSize: '15px', marginBottom: '8px', letterSpacing: '-0.01em' }}>
          {e.platform}{e.accountName && <span style={S.badge}>{e.accountName}</span>}
        </div>
        <div style={{ fontSize: '13px', color: C.textSecondary, marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
          <span style={{ minWidth: '90px', fontWeight: 500 }}>Username</span>
          <span style={{ color: C.textPrimary, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: '12.5px', padding: '2px 6px', background: C.softBg, borderRadius: '4px' }}>{e.username}</span>
          <CopyButton value={e.username} />
        </div>
        <div style={{ fontSize: '13px', color: C.textSecondary, marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
          <span style={{ minWidth: '90px', fontWeight: 500 }}>Password</span>
          <span style={{ color: C.textPrimary, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: '12.5px', padding: '2px 6px', background: C.softBg, borderRadius: '4px' }}>
            {reveal[e.id] ? e.password : '•'.repeat(Math.min((e.password || '').length, 10))}
          </span>
          <button onClick={() => setReveal({ ...reveal, [e.id]: !reveal[e.id] })} style={{ padding: '3px 10px', fontSize: '11.5px', border: `1px solid ${C.borderStrong}`, background: C.cardBg, borderRadius: '5px', cursor: 'pointer', color: C.textSecondary, fontWeight: 500 }}>{reveal[e.id] ? 'Hide' : 'Show'}</button>
        ...
        {e.recoveryEmail && (
          <div style={{ fontSize: '13px', color: C.textSecondary, marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
            <span style={{ minWidth: '90px', fontWeight: 500 }}>Recovery</span>
            <span style={{ color: C.textPrimary, fontSize: '12.5px', padding: '2px 6px', background: C.softBg, borderRadius: '4px' }}>{e.recoveryEmail}</span>
            <CopyButton value={e.recoveryEmail} />
          </div>
        )}
        {e.phone && (
          <div style={{ fontSize: '13px', color: C.textSecondary, marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
            <span style={{ minWidth: '90px', fontWeight: 500 }}>Phone</span>
            <span style={{ color: C.textPrimary, fontSize: '12.5px', padding: '2px 6px', background: C.softBg, borderRadius: '4px' }}>{e.phone}</span>
            <CopyButton value={e.phone} />
          </div>
        )}
        {e.backupCodes && (
          <div style={{ fontSize: '13px', color: C.textSecondary, marginBottom: '6px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '4px' }}>
              <span style={{ fontWeight: 500 }}>2FA backup codes</span>
              <button onClick={() => setRevealCodes({ ...revealCodes, [e.id]: !revealCodes[e.id] })} style={{ padding: '3px 10px', fontSize: '11.5px', border: `1px solid ${C.borderStrong}`, background: C.cardBg, borderRadius: '5px', cursor: 'pointer', color: C.textSecondary, fontWeight: 500 }}>{revealCodes[e.id] ? 'Hide' : 'Show'}</button>
              {revealCodes[e.id] && <CopyButton value={e.backupCodes} />}
            </div>
            {revealCodes[e.id] && (
              <pre style={{ ...S.descBox, fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: '12px', margin: 0, overflow: 'auto' }}>{e.backupCodes}</pre>
            )}
          </div>
        )}
        {e.notes && <div style={S.descBox}>{e.notes}</div>}
        {isAdmin && <AssigneeList assignees={e.assignees || []} subs={subs} />}
        <Timestamp createdAt={e.createdAt} updatedAt={e.updatedAt} />
      </div>
      {isAdmin && !selectMode && (
  ...
  return (
    <div>
      {confirmEl}
      <EditAccountModal open={!!editing} entry={editing} subs={subs} onClose={() => setEditing(null)} onSave={saveEdit} />
      {isAdmin && <AccountsEntryForm subs={subs} onSubmit={add} />}
      {entries.length > 0 && <SearchBar value={q} onChange={setQ} placeholder="Search accounts by platform, username, email…" />}
      {isAdmin && subOnlyForFilter.length > 0 && entries.length > 0 && (
        <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '1rem', alignItems: 'center' }}>
          <span style={{ fontSize: '12px', color: C.textTertiary, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', marginRight: '2px' }}>Filter:</span>
          <button onClick={() => setFilterSub('all')} className="infos-pill"
      ...
      <SelectionToolbar isAdmin={isAdmin} inSelectMode={selectMode}
        ...
        onSelectAll={() => setSelected(visible.map((e: any) => e.id))}
        onDeselectAll={() => setSelected([])} totalVisible={visible.length} />
      {visible.length === 0 ? <div style={S.empty}>{q.trim() || filterSub !== 'all' ? 'No matches found.' : 'No accounts yet. Use this tab for social media, VPN, email, and other non-game logins.'}</div> : (
        <div>
          <ReorderHint canReorder={canReorder} />
          <ReorderList items={visible} canReorder={canReorder}
            canSelect={selectMode} selectedIds={selected} onToggleSelect={toggleSelect}
...
const AccountsTab = memo(AccountsTabInner);
...
// ---------------- Tab: Create Admin ----------------
function CreateAdminPanelInner({ user, subs, setSubs, backend, games, idpass, accounts, notices, reload, reloadSubs }: any) {
  const [confirmEl, confirm] = useConfirm();
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  ...
  const remove = async (s: any) => {
    ...
    if (counts.accounts)  lines.push(`\u2022 ${counts.accounts} account${counts.accounts === 1 ? '' : 's'}`);
  ...
  const bulkEntries: BulkEntry[] = useMemo(() => {
    ...
    return [
      ...
      ...(accounts || []).map((a: any) => ({ id: a.id, label: a.platform + (a.accountName ? ` — ${a.accountName}` : ''), subLabel: `user: ${a.username}`, table: 'accounts' as const, assignees: a.assignees })),
    ];
  }, [managingSub, notices, backend, games, idpass, accounts]);
  ...
  const saveBulk = async (changes: { entry: BulkEntry; grant: boolean }[]) => {
    ...
    for (const { entry, grant } of changes) {
      ...
      else if (entry.table === 'accounts') await updateAccountAssignees(entry.id, next);
...
function SettingsModal({ open, onClose, user, onForceLogout }: any) {
  ...
  const doImport = async (ev: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      //   - v2/v3 backup → goes to current workspace, regardless of original
      ...
      const ok = await confirm({
        ...
        message: `${versionNote}\n\nThis ADDS entries from the backup. Existing entries with the same IDs will be overwritten. Zeus credentials will NOT be changed.\n\nSubs: ${data.sub_admins?.length || 0}, backend: ${data.backend_entries?.length || 0}, games: ${data.game_entries?.length || 0}, credentials: ${data.idpass_entries?.length || 0}, accounts: ${data.accounts_entries?.length || 0}, notices: ${data.notices?.length || 0}.`,
        confirmLabel: 'Import',
      });
      ...
      // v20: import accounts entries (v3 format)
      if (data.accounts_entries?.length) await bulkInsert('accounts_entries', data.accounts_entries.map((e: any) => ({
        id: e.id,
        platform: e.platform,
        account_name: e.accountName || '',
        username: e.username,
        password: e.password,
        recovery_email: e.recoveryEmail || '',
        phone: e.phone || '',
        backup_codes: e.backupCodes || '',
        notes: e.notes || '',
...
function Portal({ user, accounts, activeKey, onSwitch, onAddAccount, onSignOut, onSignOutAll, theme, setTheme }: any) {
  ...
  const getInitialTab = () => {
    ...
    try {
      ...
      const valid = ['notice', 'backend', 'games', 'idpass', 'accounts', 'admins'];
  ...
  const [accounts_, setAccounts_] = useState<any[]>([]);
  ...
  const reloaders = useMemo(() => ({
    ...
    accounts_entries: async () => { try { setAccounts_(await loadAccounts(workspaceId)); } catch (e) { console.error(e); } },
  ...
  useEffect(() => {
    ...
    setLoaded(false);
    setSubs([]); setBackend([]); setGames([]); setIdpass([]); setAccounts_([]); setNotices([]);
    (async () => {
      await reloadAll();
      if (alive) setLoaded(true);
    })();
    const unsub = subscribeAll(
      ['sub_admins', 'backend_entries', 'game_entries', 'idpass_entries', 'accounts_entries', 'notices', 'about_content'],
    ...
    <div style={S.shell}>
      ...
      <div style={S.card}>
        ...
        {!loaded ? <div style={S.empty}>Loading…</div> :
          ...
          tab === 'accounts' ? <AccountsTab user={user} subs={subs} entries={accounts_} setEntries={setAccounts_} reload={reloaders.accounts_entries} /> :
          tab === 'admins' && isAdmin ? <CreateAdminPanel user={user} subs={subs} setSubs={setSubs} backend={backend} games={games} idpass={idpass} accounts={accounts_} notices={notices} reload={reloadAll} reloadSubs={reloaders.sub_admins} /> : null}
      </div>
    </div>
  );
}
