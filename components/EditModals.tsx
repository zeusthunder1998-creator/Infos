...
export function EditIdPassModal({ open, entry, subs, onClose, onSave }: any) {
  ...
  // v20.6: Adapt label/title to the section the entry belongs to.
  // "Game" for game-credential entries, "Account" for account-credential entries.
  const isAccounts = entry.section === 'accounts';
  const primaryLabel = isAccounts ? 'Account' : 'Game';
  const modalTitle = isAccounts ? 'Edit account' : 'Edit credentials';

  const save = async () => {
    setErr('');
    if (!game.trim() || !username.trim() || !password.trim()) return setErr(`${primaryLabel}, username, and password are required`);
    if (assignees.length === 0) return setErr('Pick at least one sub-admin or "Assign to all"');
    setBusy(true);
    try {
      await onSave({
  ...
  return (
    <ModalShell title={modalTitle} onClose={onClose}
      ...
      <div style={{ display: 'grid', gap: '12px' }}>
        <div><label style={S.label}>{primaryLabel}</label><TextInput value={game} onChange={(e: any) => setGame(e.target.value)} /></div>
