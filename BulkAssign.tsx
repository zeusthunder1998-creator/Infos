...
// ---------------- v20: Edit Account (non-game credentials) ----------------
export function EditAccountModal({ open, entry, subs, onClose, onSave }: any) {
  ...
  useEffect(() => {
    if (entry) {
      ...
      setUsername(entry.username || '');
      setPassword(entry.password || '');
      setRecoveryEmail(entry.recoveryEmail || '');
      setPhone(entry.phone || '');
      setBackupCodes(entry.backupCodes || '');
  ...
  const save = async () => {
    ...
    if (assignees.length === 0) return setErr('Assign to at least one sub-admin or "Assign to all"');
    setBusy(true);
    try {
      await onSave({
    ...
    <ModalShell title="Edit account" onClose={onClose}
      ...
      <div style={{ display: 'grid', gap: '12px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          <div><label style={S.label}>Platform</label><TextInput value={platform} onChange={(e: any) => setPlatform(e.target.value)} placeholder="Facebook, NordVPN…" /></div>
          <div><label style={S.label}>Account name (optional)</label><TextInput value={accountName} onChange={(e: any) => setAccountName(e.target.value)} placeholder="Personal, Work…" /></div>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          <div><label style={S.label}>Username / Email</label><TextInput value={username} onChange={(e: any) => setUsername(e.target.value)} /></div>
          <div>
            <label style={S.label}>Password</label>
            <div style={{ position: 'relative' }}>
              <TextInput type={showPass ? 'text' : 'password'} value={password} onChange={(e: any) => setPassword(e.target.value)} style={{ paddingRight: '70px' }} />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          <div><label style={S.label}>Recovery email (optional)</label><TextInput type="email" value={recoveryEmail} onChange={(e: any) => setRecoveryEmail(e.target.value)} placeholder="backup@example.com" /></div>
          <div><label style={S.label}>Phone (optional)</label><TextInput value={phone} onChange={(e: any) => setPhone(e.target.value)} placeholder="+977…" /></div>
        </div>
        <div><label style={S.label}>2FA backup codes (optional)</label><TextArea value={backupCodes} onChange={(e: any) => setBackupCodes(e.target.value)} placeholder="Paste all backup codes here, one per line" /></div>
        <div><label style={S.label}>Notes (optional)</label><TextArea value={notes} onChange={(e: any) => setNotes(e.target.value)} /></div>
        <div><label style={S.label}>Assign to sub-admin(s)</label><AssigneePicker subs={subs} selected={assignees} onChange={setAssignees} /></div>
        {err && <div style={{ fontSize: '13px', color: C.danger, padding: '8px 12px', background: C.dangerSoft, borderRadius: '6px', fontWeight: 500 }}>{err}</div>}
      </div>
    </ModalShell>
