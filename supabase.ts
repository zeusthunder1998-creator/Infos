...
export type BulkEntry = {
  ...
  label: string;
  subLabel?: string;
  table: 'notices' | 'backend' | 'games' | 'idpass' | 'accounts';
  assignees: string[];
};
...
export function BulkAssignModal({
  ...
  const byTable = {
    ...
    accounts: filtered.filter(e => e.table === 'accounts'),
  };
    ...
    <div className="infos-modal-backdrop" onClick={handleCancel}
      ...
      <div className="infos-modal" onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '10px 14px' }}>
          ...
          {(['notices', 'backend', 'games', 'idpass', 'accounts'] as const).map((t) => {
            const items = byTable[t];
            if (items.length === 0) return null;
            const labels: Record<typeof t, string> = { notices: 'Notices', backend: 'Backend', games: 'Games', idpass: 'Id & Pass', accounts: 'Accounts' };
            return (
